...
\begin{itemize}
	...
	\item delegate state management to the compiler 
\end{itemize}
\end{frame}

\section{Asynchronous RAII}
...
\centerline{\only{\alert}<1>{What are tail recursive coroutines?}}
...
\centerline{\alert{At most 2 frames were allocated.}}
...
\begin{frame}{Why it's not implementable for task<void> types.}
	...
	\inputminted{\coroCpp}{code-examples/retval-retvoid/void_task_notail.cpp}

	\pause
	\begin{tikzpicture}[remember picture, overlay]
	\draw[thick, <-, color=blue] (4.5,3.825) -- node[right, xshift=2mm](){p.return\_void();} (5,3.825);
	\pause
